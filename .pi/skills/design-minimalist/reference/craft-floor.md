...
- **Contrast:** body and placeholder text ≥4.5:1, large text ≥3:1. On colored surfaces tint secondary text from that hue or the foreground; never use pure black (`#000000`). Prefer dark grays like `#202124` for text.
- **Depth:** shadows carry a vertical offset and a soft blur (Material elevation). A zero-offset colored halo is forbidden. Avoid harsh or unblurred borders.
- **Spacing:** tight groups, generous separation, more space above a heading than below it. Use an 8px base grid (`8px`, `16px`, `24px`, `48px`).
- **Type:** body measure 65–75ch, display max 6rem, tracking floor -0.04em. Use sans-serif fonts exclusively (Inter, Roboto, Open Sans). Run the real copy at every breakpoint and fix what overflows.
- **Motion:** one authored moment, not scattered effects and not one identical entrance on every section. Use short, subtle transitions (max `0.2s` ease-in-out) typical of Google apps.
- **States:** hover, disabled, loading, error, empty. Plus real content, working controls, responsive composition, keyboard focus. Hover states should subtly increase elevation or darken backgrounds.
...
- Gradient text. Emphasis comes from weight or size, or primary Google Blue (`#1a73e8`).
- Glass and blur as decoration. Glassmorphism is strictly forbidden in this minimalist skill. Use solid surfaces instead.
- A colored `border-left` or `border-right` above 1px on cards, list items, callouts, or alerts.
- Hard offset shadows (`box-shadow: 4px 4px 0`) or neobrutalism. Only use soft, blurred drop shadows (Material Elevation).
