...
```
node .pi/skills/design-minimalist/scripts/doctor.mjs --json
```
...
- **`auto`** carries no decision. Run `node .pi/skills/design-minimalist/scripts/doctor.mjs --fix` once to apply these, then report what it moved in one line. Do not ask permission first, and do not ask about them afterward.
