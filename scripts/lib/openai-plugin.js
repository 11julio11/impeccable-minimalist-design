...
export function stageOpenAIPlugin(rootDir, distDir) {
  ...
  const codexSkillSrc = path.join(distDir, 'codex', '.codex', 'skills', 'design-minimalist');
  ...
  const pluginRoot = path.join(distDir, 'openai', 'design-minimalist');
  ...
  fs.cpSync(
    codexSkillSrc,
    path.join(pluginRoot, 'skills', 'design-minimalist'),
    { recursive: true },
  );
